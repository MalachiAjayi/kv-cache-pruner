...
def load_model_and_tokenizer(
    ...
    if precision not in _DTYPE_MAP:
        raise ValueError(
            f"Unsupported precision: {precision}. Choose from {list(_DTYPE_MAP.keys())}."
        )
...
def load_kv_cache(
    ...
    Args:
        method: one of
            ...
            KV Cache Pruner variants (examples):
              'kvcV'  value-driven variant
              'kvcK'  key-driven variant
              'kvcVK' joint variant
    ...
    Returns:
        ...
        - An instance of `SinkCache` or `KVCachePruner` for other methods.
    """

    method = method.strip()
    # Backward compatibility with previous OBCache naming.
    if method.startswith("obc"):
        method = "kvc" + method[3:]
    ...
    if method == "sink":
        return SinkCache(num_recent=num_recent, num_heavy=num_heavy)
    ...
    kv_pruning_methods = {
        ...
        "kvcK",
        "kvcVK",
        "kvcV_p1",  # H2O and its KV-pruner-enhanced variants
        "tova",
        "kvcV+tova",
        "kvcK+tova",
        "kvcVK+tova",
        "kvcV_p1+tova",  # TOVA variants
        "snapkv",
        "kvcV+maxpool",
        "kvcK+maxpool",
        "kvcVK+maxpool",
        "kvcV_p1+maxpool",  # SnapKV variants
        "snapkv_avgpool",
        "kvcV+avgpool",
        "kvcK+avgpool",
        "kvcVK+avgpool",
        ...
        "kvcVK_no_cross",
        "kvcVK_no_cross+tova",
        "kvcVK_no_cross+maxpool",
        "kvcVK_no_cross+avgpool",
        "kvcVK_no_cross_fullhist",
    }
    if method not in kv_pruning_methods:
    ...
    elif method.startswith("kvcV"):
        use_v_score, use_vnorm = True, True
        ptb_window = num_recent if "fullhist" not in method else None
        pool_fn = (
            "maxpool"
            if "maxpool" in method
            else ("avgpool" if "avgpool" in method else None)
        )
        p = 1 if "p1" in method else 2

        if method.startswith("kvcVK"):
            use_k_score = True
